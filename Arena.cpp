...
: m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nZombies(0), m_History(nRows, nCols) //intializes History object
{
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    if (k < m_nZombies  &&  m_zombies[k]->getAttacked(dir))  // zombie dies
    {
        ...
        m_nZombies--;
        m_History.record(m_player -> row(),m_player -> col()); //calls History record function with players position
        return true;
    }
    return false;
}
...
History& Arena::history() { //public function that accessing private data member of type History by reference
    return m_History;
}
